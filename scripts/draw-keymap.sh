...
perl -0pi -e "s/CAG MOD/CAG/g; s/NAVIGATOR DEC CPI/CPI-/g; s/NAVIGATOR INC CPI/CPI+/g; s/TOGGLE SCROLL/Scroll Toggle/g; s/DRAG SCROLL/Drag Scroll/g; s/{t: ▽, h: Mouse, type: trans}/Mouse Off/g; s/OSM\\(MOD_LCTL\\)/OSM CTL/g; s/OSM\\(MOD_LALT\\)/OSM ALT/g; s/OSM\\(MOD_LGUI\\)/OSM GUI/g; s/OSM\\(MOD_LSFT\\)/OSM SFT/g" "$tmp_yaml"
